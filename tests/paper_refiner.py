...
async def run_latex_paper_optimization(project_dir_str: str, main_file: str = "main.tex"):
    ...
    instruction = f"""
    OBJECTIVE: Perform a comprehensive optimization of the LaTeX project for ICML 2026 submission standards.
    
    TARGET MAIN FILE: {main_file}
    
    WORKING PROTOCOL:
    1. **Project Mapping**: Use `edit.list_tree` for a physical file overview and `latex_editor.map_project` to understand the logical inclusion hierarchy.
    2. **Citation Audit**: Run `latex_editor.check_citations` to identify missing or unused references. Refine the .bib entries to ensure high academic standards.
    3. **Mathematical Consistency**: Use `latex_editor.view_tex_env` to audit Methodology sections. Ensure notation consistency and professional formatting of equations and algorithms.
    4. **Iterative Refinement**: Polish core sections (Abstract, Intro, Method). After each significant edit, you MUST call `latex_editor.compile_and_diagnose`. Zero compilation errors is a mandatory requirement.
    5. **Strategic Logging**: Record all major improvements, logical refinements, and space-saving techniques in `editing_log.md` using `latex_editor.append_edit_rationale`.
    
    GOAL: Enhance the paper's logical flow and technical professionality. Ensure the final project is bug-free, fully cited, and ready for top-tier review. 
    Call `done` after verifying the final build and summarizing your key enhancements.
    """
